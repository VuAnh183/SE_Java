...
public class ProgStudentMan {
  ...
  public String toString() {
	  ...
	  // Iterator is an object that can be used to loop through collections (in this case a TreeSet)
	  // Using iterator() to get an iterator from the collection object = TreeSet<Student>  
	  Iterator<Student> it = objects.iterator();
  ...
  public Student createStudent(Object[] data) throws NotPossibleException{
	  // Ensure data has exactly 4 elements
	  if(data == null || data.length != 4) {
		  throw new NotPossibleException("Data must contain exactly 4 elements: id, name, phone, address!");
	  }
  ...
  public UndergradStudent createUndergradStudent(Object[] data) throws NotPossibleException {
	  ...
	  // Create new UndergradStudent from the input data
  ...
  public PostgradStudent createPostgradStudent(Object[] data) throws NotPossibleException{
	  // Ensure data has exactly 5 elements
	  if(data == null || data.length != 5) {
		  throw new NotPossibleException("Data must contain exactly 5 elements: id, name, phone, address, gpa!");
	  }
	  
	  // Create new PostgradStudent from the input data
	  return new PostgradStudent(
			  (int) data[0],
			  (String) data[1],
			  (String) data[2],
			  (String) data[3],
  ...
  public Query search(String[] words) throws NotPossibleException{
	  if(words == null || words.length == 0) {
		  throw new NotPossibleException("Search words cannot be null or empty!");
	  }
	  
	  Query query = new Query();
	  
	  for(String word: words) {
		  query = engine.queryMore(word);
	  }
	  return query;
